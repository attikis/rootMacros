...
def main():
    ...
    # Create Functions
    lambdaParam = "1.2"
    expr = "%s*exp(-%s*x)" % (lambdaParam, lambdaParam)
    f1 = ROOT.TF1("Exponential_1", expr, 0.0 , 3.0 )
    hList.append(f1)

    expr = "%s*exp(-%s*(x-0.1))" % (lambdaParam, lambdaParam)
    f2 = ROOT.TF1("Exponential_2", expr, 0.0 , 3.0 )
    hList.append(f2)

    expr = "%s*exp(-%s*(x+0.1))" % (lambdaParam, lambdaParam)
    f3 = ROOT.TF1("Exponential_3", expr, 0.0 , 3.0 )
    hList.append(f3)
