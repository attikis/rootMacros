...
def main():
    ...
    for i in range(0, nHistos):
        h = CreateTH1F(hName = "Binomial_" + str(i), hTitle = "Binomial_" + str(i), binWidthX = 0.5, xMin = 0.0, xMax = 70.0, xMean = mean[i]) 
        hList.append(h)
...
def DrawHistoList(hList, fNormalisationFactor=None):
    ...
    for h in hList:
        nBinsX = h.GetNbinsX()
        myLeg.AddEntry(h, "p = %s , n = %s" % (str(p[counter]), str(nTrials[counter])))
