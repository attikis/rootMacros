...
# Global definitions
hList      = []
#lambdaVals = [0.5, 5, 10, 20, 35]
lambdaVals = [0.1, 0.5, 2, 5]
nTrials    = [100, 100, 100, 100]
#nTrials    = [20, 20, 20, 20]
...
def main():
    ...
    for l, colour, n in zip(lambdaVals, colours, nTrials):
        ...
        h = CreateTH1F(hName = "Poisson_%i" % (counter), hTitle = "Poisson_%i" % (counter), binWidthX = 1.0, xMin = -1.0, xMax = 15.0, xMean = None) 
        h.SetTitle("")
...
def DrawHistoList(hList, fNormalisationFactor=None):
    ...
    # Draw histograms
    for h in hList:
        #myLeg.AddEntry(h, "#lambda = %0.1f, n = %i, n#lambda = %0.0f" % (lambdaVals[counter], nTrials, lambdaVals[counter]*nTrials), "l")
        myLeg.AddEntry(h, "#lambda = %0.2f, n = %i" % (lambdaVals[counter], nTrials[counter]), "f")
