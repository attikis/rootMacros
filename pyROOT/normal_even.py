...
def main():
    ...
    x = "(x)"
    xSquared = "(x^2)"
    expr = "%s*%s*exp(-0.5*((%s-%s)/(%s))^2)" % (a, xSquared, x, mean1, variance)
    f1 = ROOT.TF1("f1", expr, -20.0 , 20.0 )
    hList.append(f1)

    mean2 = "10"
    expr2 = "%s*exp(-0.5*((%s-%s)/(%s))^2)" % (a, xSquared, mean2, variance)
    f2 = ROOT.TF1("f2", expr2, -20.0 , 20.0 )
    hList.append(f2)

    mean3 = "0"
    expr3 = "%s*exp(-0.5*((%s-%s)/(%s))^2)" % (a, xSquared, mean3, variance)
    f3 = ROOT.TF1("f3", expr3, -20.0 , 20.0 )
    hList.append(f3)

    mean4 = "0"
    expr4 = "%s*%s*exp(-0.5*((%s-%s)/(%s))^2)" % (a, xSquared, x, mean4, variance)
    f4 = ROOT.TF1("f4", expr4, -20.0 , 20.0 )
    hList.append(f4)

    mean5 = "-10"
    expr5 = "%s*%s*exp(-0.5*((%s-%s)/(%s))^2)" % (a, xSquared, x, mean5, variance)
