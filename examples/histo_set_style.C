...
void histo_draw_options(){
  ...
  // Use gStyle to set GLOBAL variables! (this applies to ALL histograms!)
  // gStyle-> SetHistFillColor(kBlue);
  // gStyle-> SetHistFillStyle(5);
  // gStyle-> SetHistLineColor(kRed);
  // gStyle-> SetHistLineWidth(1.2);
  // gStyle-> SetHistLineStyle(5);
  
  // To apply these new GLOBAL changes to the histograms use:
  // hGaus->UseCurrentStyle();
  // hExp->UseCurrentStyle();
  // Note that this must be done to  each histogram individually!
  
  // To manually set Style to individual histograms (and not globally) 
  // use instead this :
  hBreitWigner->SetFillColor(kRed);
  hBreitWigner->SetLineColor(kBlue);

  hLandau->SetFillColor(kYellow);
  hLandau->SetLineWidth(2);

  hExp->SetFillColor(kGreen);
  hExp->SetLineStyle(2);
  hExp->SetLineWidth(3);

  hGaus->SetFillColor(kBlue);
