...
void test(void) {
  ...
  qcd.AddText("default", 0.8, 0.19, 0.9 );
  //////////////////////////////////////////////    
  qcd.AddVerticalLine(80.0, yMin, 1E04);
  ...
  //////////////////////////////////////////////  //////////////////////////////////////////////
  HistoName = "mT_Tail_2";
  TString MyTCut2 = Met + " && " + DeltaPhiTauMet + " && " + PassBtag  + " && " + NFwdJets;
  // MyTCut = Met + " && " + DeltaPhiTauMet + " && " + PassBtag  + " && " + NFwdJets + " && " + NBjetsFail;
  // MyTCut = Met + " && " + DeltaPhiTauMet + " && " + PassBtag  + " && " + NBjetsFail;
  qcd2.Initialize(MyDatasets, MulticrabDirPath, FolderName, TreeName, BranchName, MyTCut2, HistoName, nBins, xMin, xMax,  MyEventWeight, bApplyEventWeight);
  ...
  qcd2.AddText("tail suppressed", 0.8, 0.19, 0.9 );
  ...
  qcd2.AddText("tail suppressed", 0.8, 0.19, 0.9 );
  //////////////////////////////////////////////    
  qcd2.AddVerticalLine(80.0, yMin, yMax);
  ...
  HistoName = "mT_Tail_3";
  MyTCut = "";
  qcd3.Initialize(MyDatasets, MulticrabDirPath, FolderName, TreeName, BranchName, MyTCut, HistoName, nBins, xMin, xMax,  MyEventWeight, bApplyEventWeight);
  qcd3.SetHPlusBR(0.05);
  qcd3.ReadLumiesFromJSON("lumi.json");
  qcd3.PrepareHistos();
  qcd3.Draw(drawOptions, false);
  qcd3.AddLegend("", xMinLeg, yMinLeg, xMaxLeg, yMaxLeg, legOptions);
  qcd3.AddText("cut away", 0.8, 0.19, 0.9 );
  qcd3.CustomizeHisto(xMin, xMax, yMin, yMax, xTitle, yTitle + qcd3.GetHistoBinWidth(1, "%.0f") + " " + units, true);
  qcd3.PrintHistoInfo(1);  
  ///
  TH1D *hData3 = (TH1D*) hData->Clone("hData3");
  hData3->Add(hData2, -1);
  qcd3.SetTH1DStyle(hData3, kBlack, 1001, kBlack, kSolid, 2 , kBlack, kFullCircle, 1.2);
  // qcd3.DrawHisto(hData3, xMin, xMax, yMin, yMax, xTitle , yTitle+ qcd3.GetHistoBinWidth(1, "%.0f") + " " + units, true, "p", HistoName, false);
  qcd3.AddDefaultText();
  ///
  TH1D *hDataMinusEwk3 = (TH1D*) hDataMinusEwk->Clone("hDataMinusEwk3");
  hDataMinusEwk3->Add(hDataMinusEwk2, -1);
  qcd3.SetTH1DStyle(hDataMinusEwk3, kOrange, 1001, kOrange, kSolid, 2 , kOrange, kFullTriangleUp, 1.2);
  // qcd3.DrawHistoSame(hDataMinusEwk3, "p");
  qcd3.DrawHisto(hDataMinusEwk3, xMin, xMax, yMin, yMax, xTitle , yTitle+ qcd3.GetHistoBinWidth(1, "%.0f") + " " + units, true, "p", HistoName, false);
  ///
  TH1D *hEwk3 = (TH1D*) hEwk->Clone("hEwk3");
  hEwk3->Add(hEwk2, -1);
  qcd3.SetTH1DStyle(hEwk3, kRed, 1001, kRed, kSolid, 2 , kRed, kFullSquare, 1.2);
  // qcd3.DrawHistoSame(hEwk3, "p");
  ///
  TH1D *hSignal3 = (TH1D*) hSignal->Clone("hSignal3");
  hSignal3->Add(hSignal2, -1);
  qcd.SetTH1DStyle(hSignal3, kOrange, 1001, kBlue, kSolid, 2 , kBlue, kFullTriangleDown, 1.2);
  qcd3.DrawHistoSame(hSignal3, "p");
  //////////////////////////////////////////////    
  TLegend *myLegend3 = qcd3.CreateLegend("", xMinLeg, xMaxLeg, yMinLeg, yMaxLeg );
  // myLegend3->AddEntry(hData3, "Data", "p" );
  // myLegend3->AddEntry(hDataMinusEwk3, "Data - Ewk", "p" );
  // myLegend3->AddEntry(hEwk3, "Ewk", "p" );
  // myLegend3->AddEntry(hSignal3, "m_{H^{#pm}} = " + signalMass + " " + units, "p" );
  myLegend3->AddEntry(hDataMinusEwk3, "QCD", "p" );
  myLegend3->AddEntry(hSignal3, "m_{H^{#pm}} = " + signalMass + " " + units, "p" );
  myLegend3->Draw();
  qcd3.AddText("cut away", 0.8, 0.19, 0.9 );
  //////////////////////////////////////////////    
  qcd3.AddVerticalLine(80.0, yMin, yMax);
  // qcd3.AddCutBox(0, yMin, 80.0, yMax);
  //////////////////////////////////////////////    
