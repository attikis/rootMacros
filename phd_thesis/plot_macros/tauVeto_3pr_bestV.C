...
#include <TH1D>
  ...
  checkTFile( myFile );
   ...
   // signal: MC matched taus from W->taunu decay, tau->pi+- pi0s, 3prong
    ...
    //
    v_myCanvas[counter]->cd();
    Super2LeafsFromTree_Norm( myFile, "TauID_TTbar", "pfElectronPt", "pfElectronPt", bkgCut, signalCut, 0, 30, 1, "hadr. jets", "#tau-jets (3pr)", "test", "#sum PFe p_{T} in jet (GeV)" );
    AddTText_PRELIM()->Draw();
    AddLatexText("");
    counter++;
    // 
    v_myCanvas[counter]->cd();
    Super2LeafsFromTree_Norm( myFile, "TauID_TTbar", "pfNeuHadrPt", "pfNeuHadrPt", bkgCut, signalCut, 0, 50, 1, "hadr. jets", "#tau-jets (3pr)", "test", "#sum PF neut. hadr p_{T} in jet (GeV)" );
    AddTText_PRELIM()->Draw();
    AddLatexText("");
    counter++;
    //
    v_myCanvas[counter]->cd();
    Super2LeafsFromTree_Norm( myFile, "TauID_TTbar", "deltaE", "deltaE", bkgCut, signalCut, -1, 0.2, 1, "hadr. jets", "#tau-jets (3pr)", "test", "Fraction of E_{T} lost by neut. hadr." );
    AddTText_PRELIM()->Draw();
    AddLatexText("");
    counter++;
    // 
    ...
    //
    v_myCanvas[counter]->cd();
    Super2LeafsFromTree_Norm( myFile, "TauID_TTbar", "flightpathSig", "flightpathSig", bkgCut, signalCut, -100, 100, 1, "hadr. jets", "#tau-jets (3pr)", "test", "Significance of reco-tau path" );
    AddTText_PRELIM()->Draw();
    AddLatexText("");
    counter++;
    //
    ...
    //
//     v_myCanvas[counter]->cd();
//     Super2LeafsFromTree_Norm( myFile, "TauID_TTbar", "signalExcessPt", "signalExcessPt", bkgCut, signalCut, 0, 200, 1, "hadr. jets", "#tau-jets (3pr)", "test","signalExcessPt (GeV/c)" );
//     AddTText_PRELIM()->Draw();
//     AddLatexText("");
//     counter++;
    //
