...
void BrHPlus(void) {
  ...
  if(TanBetaValue.CompareTo("30")==0) v_MyBranch.push_back("BR_Hp_cb");
  ...
  if(TanBetaValue.CompareTo("30")==0) v_MyBranch.push_back("BR_Hp_us");
  v_MyBranch.push_back("BR_Hp_NeuAChaA+BR_Hp_NeuBChaA+BR_Hp_NeuCChaA+BR_Hp_NeuDChaA+BR_Hp_NeuAChaB+BR_Hp_NeuBChaB+BR_Hp_NeuCChaB");
  //
  // v_MyBranch.push_back("BR_Hp_ud");
  // v_MyBranch.push_back("BR_Hp_cd");
  // v_MyBranch.push_back("BR_Hp_ub");
  // v_MyBranch.push_back("BR_Hp_enu");
  // v_MyBranch.push_back("BR_Hp_HW");
  ...
  if(TanBetaValue.CompareTo("30")==0) v_MyBranchName.push_back("  b c  ");
  ...
  if(TanBetaValue.CompareTo("30")==0) v_MyBranchName.push_back("  u s  ");
  v_MyBranchName.push_back("  #tilde{#chi}^{0}_{i} #tilde{#chi}^{#pm}_{j}  ");
  //
  // v_MyBranchName.push_back("  u d  ");
  // v_MyBranchName.push_back("  c d  ");
  // v_MyBranchName.push_back("  u b  ");
  // v_MyBranchName.push_back("  e #nu_{e}  ");
  // v_MyBranchName.push_back("  H^{#pm} W^{#pm}  ");
  ...
  for(int i = 0; i < NGraphs; i++, it_MyBranchName++, ColourCounter++){
	...
	yLegendPos = yLegendPos*1.5;
      }
      if( ((TString)graph->GetName()).Contains("_munu") ){
	xLegendPos = xLegendPos*0.9;
