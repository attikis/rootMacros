...
void BrHPlus_TanThree(void) {
  ...
  for(int i = 0; i < NGraphs; i++, it_MyBranchName++, ColourCounter++){
    ...
    if( ((TString)graph->GetName()).Contains("_A0W") ){
      ...
      xLegendPos = xLegendPos*0.68;
      yLegendPos = yLegendPos*1.6;
    }
    if( ((TString)graph->GetName()).Contains("_h0W") ){
      xLegendPos = xLegendPos*0.98;
      yLegendPos = yLegendPos*0.78;
    }
    ...
    if( ((TString)graph->GetName()).Contains("_us") ){
      ...
      yLegendPos = yLegendPos*2.8;
    }
    if( ((TString)graph->GetName()).Contains("_munu") ){
      xLegendPos = xLegendPos*0.45;
      yLegendPos = yLegendPos*36.0;
    }
