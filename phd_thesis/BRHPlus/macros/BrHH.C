...
void BrHH(void) {
  ...
  TLegend *MyMgraphLegend = plotter.CreateLegend("  H^{0} #rightarrow X  ", xLegPosMin, xLegPosMax, yLegPosMin, yLegPosMax); // decay modes
  ...
  v_MyBranch.push_back("BR_H_hh");
  v_MyBranch.push_back("BR_H_tt");
  v_MyBranch.push_back("(BR_H_ChaAChaA>0 ? BR_H_ChaAChaA:0) + (BR_H_ChaAChaB>0 ? BR_H_ChaAChaB:0)+ (BR_H_ChaBChaA>0 ? BR_H_ChaBChaA:0)");
  v_MyBranch.push_back("(BR_H_NeuANeuA>0 ? BR_H_NeuANeuA:0) + (BR_H_NeuANeuB>0 ? BR_H_NeuANeuB:0) + (BR_H_NeuANeuC>0 ? BR_H_NeuANeuC:0) + (BR_H_NeuANeuD>0 ? BR_H_NeuANeuD:0) + (BR_H_NeuBNeuB>0 ? BR_H_NeuBNeuB:0) + (BR_H_NeuBNeuC>0 ? BR_H_NeuBNeuC:0) + (BR_H_NeuBNeuD>0 ? BR_H_NeuBNeuD:0) + (BR_H_NeuCNeuC>0 ? BR_H_NeuCNeuC:0) + (BR_H_NeuCNeuD>0 ? BR_H_NeuCNeuD:0)");
  if(TanBetaValue.CompareTo("3") == 0) v_MyBranch.push_back("BR_H_ZZ");  
  //
  // v_MyBranch.push_back("BR_H_cc");
  // v_MyBranch.push_back("BR_H_gammagamma");
  // v_MyBranch.push_back("BR_H_AZ");
  // v_MyBranch.push_back("BR_H_hA");
  // v_MyBranch.push_back("BR_H_AA");
  ...
  v_MyBranchName.push_back("  h^{0} h^{0}  ");
  ...
  if(TanBetaValue.CompareTo("3") == 0) v_MyBranchName.push_back("  Z^{0} Z^{0}  ");
  //
  // v_MyBranchName.push_back("  c #bar{c}  ");
  // v_MyBranchName.push_back("  #gamma #gamma  ");
  // v_MyBranchName.push_back("  H^{0} Z^{0}  ");
  // v_MyBranchName.push_back("  h^{0} H^{0}  ");
  // v_MyBranchName.push_back("  H^{0} H^{0}  ");
  ...
  for(it_MyBranch = v_MyBranch.begin(); it_MyBranch < v_MyBranch.end(); it_MyBranch++, it_MyBranchName++, ColourCounter++,StyleCounter++){
    ...
    for(it_MassPoints = v_MassPoints.begin(); it_MassPoints < v_MassPoints.end(); it_MassPoints++, MassCounter++){
      ...
      // Create Legend
      TLegend *BrHistoLegend = plotter.CreateLegend("H^{0} #rightarrow X", xLegPosMin, xLegPosMax, yLegPosMin, yLegPosMax);
      ...
      plotter.AddText("m_{h}^{max} scenario", TextResizeValue, xTextPos+(0.25*2), yTextPos); 
      // Create Legend
      TLegend *MassHistoLegend = plotter.CreateLegend("m_{H^{0}}", xLegPosMin, xLegPosMax, yLegPosMin, yLegPosMax);
  ...
  for(int i = 0; i < NGraphs; i++, it_MyBranchName++, ColourCounter++){
	...
	xLegendPos = xLegendPos*0.4;
	yLegendPos = yLegendPos*1.2;
    }
      if( ((TString)graph->GetName()).Contains("_ss") ){
	xLegendPos = xLegendPos*0.85;
	yLegendPos = yLegendPos*1.4;
      }
    }

    // Exceptions here
    if(TanBetaValue.CompareTo("10")==0){
      if( ((TString)graph->GetName()).Contains("_WW") ){
	xLegendPos = xLegendPos*0.43;
	yLegendPos = yLegendPos*0.84;
    }
      if( ((TString)graph->GetName()).Contains("_mumu") ){
	xLegendPos = xLegendPos*0.8;
	yLegendPos = yLegendPos*1.1;
      }
      if( ((TString)graph->GetName()).Contains("_NeuANeuA") ){
	xLegendPos = xLegendPos*1.0;
	yLegendPos = yLegendPos*0.8;
      }
      if( ((TString)graph->GetName()).Contains("_gg") ){
	xLegendPos = xLegendPos*0.92;
	yLegendPos = yLegendPos*1.1;
      }
    }
	...
	yLegendPos = yLegendPos*1.4;
      ...
      if( ((TString)graph->GetName()).Contains("_hh") ){
	xLegendPos = xLegendPos*0.5;
	yLegendPos = yLegendPos*0.9;
      }
    }
  ...
  if(bSavePlots) plotter.SaveAs(tanb+TanBetaValue+"/BrHHVsMass");
  if(bSavePlots) plotter.SaveAs("/Users/administrator/my_work/latex/ucy/PhD/MyPhDThesis/figures/theory/mssm/BRHPlus/"+tanb+TanBetaValue+"/BrHHVsMass");
