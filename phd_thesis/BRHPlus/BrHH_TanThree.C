...
void BrHH_TanThree(void) {
  ...
  const TString TanBetaValue = "3"; // 3, 7, 10, 30
  ...
  for(int i = 0; i < NGraphs; i++, it_MyBranchName++, ColourCounter++){
    ...
    if( ((TString)graph->GetName()).Contains("_WW") ){
      ...
      yLegendPos = yLegendPos*0.65 ;
    }
    if( ((TString)graph->GetName()).Contains("_hh") ){
      xLegendPos = xLegendPos*0.99;
      yLegendPos = yLegendPos*1.0;
    }
    if( ((TString)graph->GetName()).Contains("_ZZ") ){
      xLegendPos = xLegendPos*0.99;
      yLegendPos = yLegendPos*0.86;
    }
  ...
  if(bSavePlots) plotter.SaveAs("/Users/administrator/Desktop/BrHHVsMass");
