...
void BrA_TanThree(void) {
  ...
  for(int i = 0; i < NGraphs; i++, it_MyBranchName++, ColourCounter++){
    ...
    if( ((TString)graph->GetName()).Contains("_tt") ){
      graph->GetPoint(nGraphBins/2, xLegendPos, yLegendPos);
      xLegendPos = xLegendPos*0.8;
      yLegendPos = yLegendPos*1.2;
    }
    if( ((TString)graph->GetName()).Contains("_hZ") ){
      xLegendPos = xLegendPos*0.99;
      yLegendPos = yLegendPos*0.75;
    }
    if( ((TString)graph->GetName()).Contains("_ss") ){
      xLegendPos = xLegendPos*0.65;
      yLegendPos = yLegendPos*4.0;
    }
    if( ((TString)graph->GetName()).Contains("_mumu") ){
      xLegendPos = xLegendPos*0.8;
      yLegendPos = yLegendPos*1.3;
    }
    if( ((TString)graph->GetName()).Contains("_Cha") ){
      xLegendPos = xLegendPos*0.99;
      yLegendPos = yLegendPos*1.2;
    }
    if( ((TString)graph->GetName()).Contains("_Neu") ){
      xLegendPos = xLegendPos*0.99;
