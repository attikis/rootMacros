...
public :
 ...
 //edm::Wrapper<vector<float> > *floats_fixedConePFTaus_trackIsolationUsingLeadingPion_HChSignalAnalysis.;
   ...
   alpha_T(const std::string SampleName, TTree *tree=0);
   virtual ~alpha_T();
...
#ifdef alpha_T_cxx
alpha_T::alpha_T(const std::string SampleName, TTree *tree)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
     ...
     tree = mychain;
     std::cout << "Number of events read "<<"\t" << mychain->GetEntries() << std::endl;
   }
   Init(tree);
}
