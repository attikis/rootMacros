...
// Mon Aug 16 14:57:04 2010 by ROOT version 5.22/00d
// from TTree Events/
...
#include "EvtTopology.C"
#include "MyJet.C"
#include "MyVector3D.C"
#include "VectorCut.C"
#include "Plotter.C"
...
struct  JetStruc_t{
  ...
  Float_t fPhi;
  Bool_t  bLdgTrk;
  Float_t fLdgTtk_Momentum;
  Float_t fLdgTtk_Pt;
  Float_t fLdgTtk_Eta;
  Float_t fLdgTtk_Phi;
  Bool_t  bLdgTrkIsol;
  Float_t fLdgTtkIsol_Momentum;
  Float_t fLdgTtkIsol_Pt;
  Float_t fLdgTtkIsol_Eta;
  Float_t fLdgTtkIsol_Phi;
};
...
public :
   ...
   Plotter oPlotter; 
   MetStruc_t sMet;  
   AlphaStruc_t sAlpha_calo;
   AlphaStruc_t sAlpha_jpt;
   bool bCut_BasicJptJets;
   bool bCut_BasicCaloJets;
   bool bCut_Acceptance;
 ...
 //edm::Wrapper<vector<float> > *floats_fixedConePFTaus_trackIsolationUsingLeadingPion_HChSignalAnalysis.;
   ...
   bareROOTmacro(const TString RootFileName, TTree *tree=0);
...
bareROOTmacro::bareROOTmacro(const TString RootFileName, TTree *tree){
...
/*       tree = (TTree*)gDirectory->Get("Events"); */
      ...
      // Describe actions to be taken after TChain is loaded (using external file "run.C")
   Init(tree);

   //   TFile *MyFile = new TFile(RootFileName,"UPDATE"); // open an existing file for writing.If no file exists, it is created.
   TFile *MyFile = new TFile(RootFileName,"RECREATE"); // create a new file, if the file already exists it will be overwritten.

   if ( MyFile->IsOpen() ) printf("*** File opened successfully!\n");
   // Once a TFile object has been created it becomes the default file for all I/O. 
   // This default is held in the global variable gFile, which can be updated at any time to change the default, e.g.:
   // gFile = MyFile;
}
...
{
   if (!fChain) return;
   // delete fChain->GetCurrentFile(); // causes segmentation violation
}
